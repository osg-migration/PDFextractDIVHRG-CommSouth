...
#Region "OSG Process"
    ...
    Private Sub standardProcessing()
        ...
        Else
            CurrentPDFFileName = "D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\DIVHRG-CommSouth.pdf"
            clientCode = "DIVHRGCommSo"
            ...
            docuFileName = "D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\Output\swdocu.txt"
            swSLCTFileName = "D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\Output\swSLCT.txt"
            workDir = "D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\Output"
            xmlOut = New XmlTextWriter("D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\Output\result.xml", enc)
    ...
    Private Sub ProcessPDF()
        ...
        Using inDoc As New PDFDoc(CurrentPDFFileName)
            ...
            Else
                inDoc.Save("D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\Output\updated.pdf", SDF.SDFDoc.SaveOptions.e_compatibility + SDF.SDFDoc.SaveOptions.e_remove_unused)
            End If
    ...
    Private Sub ProcessPage(inDoc As PDFDoc)
        ...
        'AdjustPagePosition(CurrentPage, 0, 0)
    ...
    Private Sub ProcessPage1()
        ...
        'Start log account info
        convLog.addEntry("Start - Account#", accountNumber, 0)
        convLog.addEntry("Document Date", docDate, 1)
        convLog.addEntry("Due Date", DueDate, 1)
        convLog.addEntry("Total Due", totalAmtDue, 1)

        'White Out Address Box
        WhiteOutContentBox(0.5, 0.75, 4, 1.25, , , , 1)
        'Remove Logo
        WhiteOutContentBox(0.56, 10.39, 1.97, 0.55, , , , 1)
        'Add top logo 
...
#Region "Standard PDF Procedures"
    ...
    Private Sub CreateCropPage()
        ...
        Else
            cropPDF = "D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\Output" & "\crop.pdf"
        End If

        If (OSG) Then
            cropDoc = New PDFDoc()
        Else
            cropDoc = New PDFDoc("D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth\DIVHRG-CommSouth.pdf")
        End If
    ...
    Private Sub LoadOverlays()
        ...
        'PDF Logo
        If (OSG) Then
            File.Copy(FRMW.getParameter("FRAMEWORKCLIENTPDFRESOURCESDIR") & "\HAR-012 j68253 Hargray LH.pdf", workDir & "\DIVHRG-CommSouth.PDF", True)
            'File.Copy(FRMW.getParameter("FRAMEWORKCLIENTPDFRESOURCESDIR") & "\DIVCOH-BACK.PDF", workDir & "\DIVCOH-BACK.PDF", True)
        Else
            File.Copy("D:\OSGMigration\Git\PDFextractDIVHRG-CommSouth" & "\HAR-012 j68253 Hargray LH.pdf", workDir & "\DIVHRG-CommSouth.PDF", True)
            ' File.Copy("D:\OSGMigration\Git\PDFextractDIVHRG-Commerical\Output" & "\DIVCOH-BACK.PDF", workDir & "\DIVCOH-BACK.PDF", True)
        End If

        overlayFrontDoc = New PDFDoc(workDir & "\DIVHRG-CommSouth.PDF")
    ...
    Private Sub AddImage(PDFpage As Page, imageKey As String, xPosition As Double, yPosition As Double)
        ...
        EW.Begin(PDFpage, ElementWriter.WriteMode.e_overlay)
